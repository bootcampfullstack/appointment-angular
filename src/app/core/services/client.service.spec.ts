...
describe('ClientService', () => {
  ...
  it('should fetch clients page', () => {
    ...
    service.getClientsPage(nameFilter, page).subscribe( response => {
       expect(response).toEqual(mockPage);
    });

    const req = httpMock.expectOne(`${service.baseUrl}?name_like=${nameFilter}&_page=${page}&_limit=10&_sort=name`);
  ...
  it('should get client by ID', () => {
    const clientId = 1;
    const mockClient: Client = { id: clientId, name: 'Test Client', phone: '123456789',  dateOfBirth: '2020-10-10', comments: '' };
